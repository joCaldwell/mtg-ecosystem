...
type TriggerSubject = { what: ObjectRef; grouping?: "one-or-more" };

function parseTriggerSubject(c: Cursor): TriggerSubject | null {
  return c.attempt((c): TriggerSubject | null => {
    if (c.word("a", "an") !== null) {
      const filter = parseFilter(c);
      if (!filter) return null;
      return { what: { ref: "filter", filter, count: { amount: "fixed", value: 1 } } };
    }
    if (c.attempt((c) => (c.words("one", "or", "more") ? true : null))) {
      const filter = parseFilter(c);
      if (!filter) return null;
      return { what: { ref: "filter", filter, count: { amount: "fixed", value: 1 } }, grouping: "one-or-more" };
    }
    ...
    if (bare) return { what: { ref: "filter", filter: bare, count: { amount: "fixed", value: 1 } } };
...
function parsePhase(c: Cursor): PhaseName | null {
  return c.attempt((c): PhaseName | null => {
...
export function parseTrigger(c: Cursor): Trigger | null {
  return c.attempt((c): Trigger | null => {
    ...
    const parsedSubject = parseTriggerSubject(c);
    if (!parsedSubject) return c.fail("trigger subject");
    const { what: subject, grouping } = parsedSubject;
    ...
    if (c.word("enters", "enter") !== null) {
      ...
      if (c.word("or") !== null) {
        if (c.word("leaves", "leave") !== null) {
          c.words("the", "battlefield");
          return { ...(grouping ? { grouping } : {}), trigger: "enters-or-leaves", what: subject };
        }
        if (c.word("attacks", "attack") !== null) return { ...(grouping ? { grouping } : {}), trigger: "enters-or-attacks", what: subject };
        if (c.word("dies", "die") !== null) return { ...(grouping ? { grouping } : {}), trigger: "enters-or-dies", what: subject };
        return null;
      }
      return { ...(grouping ? { grouping } : {}), trigger: "enters", what: subject };
    }
    if (c.word("leaves", "leave") !== null) {
      c.words("the", "battlefield");
      return { ...(grouping ? { grouping } : {}), trigger: "leaves", what: subject };
    }
    if (c.word("dies", "die") !== null) return { ...(grouping ? { grouping } : {}), trigger: "dies", what: subject };
    if (c.attempt((c) => (c.words("is", "put", "into") ? true : null))) {
      const owner = c.word("a", "your");
      if (!owner || c.word("graveyard") === null) return null;
      const from = c.words("from", "the", "battlefield") ? "battlefield"
        : c.words("from", "anywhere") ? "anywhere" : null;
      if (!from) return c.fail("graveyard event source");
      return { ...(grouping ? { grouping } : {}), trigger: "put-into-graveyard", what: subject,
        from, to: { zone: "graveyard", owner: owner === "your" ? "your" : "any" } };
    }
    ...
    if (c.word("attacks", "attack") !== null) {
      ...
      return { ...(grouping ? { grouping } : {}), trigger: "attacks", what: subject, alone: alone || undefined, whom: whom.length ? whom : undefined };
    }
    if (c.word("blocks") !== null) {
      if (c.attempt((c) => (c.words("or", "becomes", "blocked") ? true : null)))
        return { ...(grouping ? { grouping } : {}), trigger: "blocks", what: subject, orBecomesBlocked: true };
      const blocking = c.attempt(parseObjectRef);
      return { ...(grouping ? { grouping } : {}), trigger: "blocks", what: subject,
        ...(blocking ? { blocking } : {}) };
    }
    if (c.words("becomes", "blocked")) {
      ...
      return { ...(grouping ? { grouping } : {}), trigger: "becomes-blocked", what: subject, by: by ?? undefined };
    }
    if (c.words("becomes", "tapped")) return { ...(grouping ? { grouping } : {}), trigger: "becomes-tapped", what: subject };
    if (c.words("becomes", "untapped")) return { ...(grouping ? { grouping } : {}), trigger: "becomes-untapped", what: subject };
    if (c.attempt((c) => (c.words("becomes", "the", "target", "of") ? true : null))) {
      c.word("a", "an");
      const of = parseFilter(c);
      if (!of) return null;
      return { ...(grouping ? { grouping } : {}), trigger: "becomes-target", what: subject, of };
    }
    if (c.attempt((c) => (c.words("is", "dealt", "damage") ? true : null)))
      return { ...(grouping ? { grouping } : {}), trigger: "is-dealt-damage", what: subject };
    if (c.word("deals") !== null) {
      ...
      return { ...(grouping ? { grouping } : {}), trigger: "deals-damage", what: subject, combat: combat || undefined, to };
    }
    if (c.attempt((c) => (c.words("is", "tapped", "for", "mana") ? true : null)))
      return { ...(grouping ? { grouping } : {}), trigger: "taps-for-mana", what: subject };
