...
import type { Condition, ObjectRef, StaticEffect, StaticModifier } from "../ast.ts";
...
function parseStaticModifiers(c: Cursor): StaticModifier[] | null {
  return c.attempt((c): StaticModifier[] | null => {
...
export function parseStatic(c: Cursor): StaticEffect | null {
  return c.attempt((c): StaticEffect | null => {
